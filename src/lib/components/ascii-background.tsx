...
const TECH_WORDS = [
  "growth_hack", "unicorn", "hockey_stick", "north_star", "cohort",
  "churn_rate", "burn_rate", "runway", "pivot", "MVP", "PMF",
  "conversion", "retention", "activation", "referral", "revenue",
  "DAU", "MAU", "WAU", "CAC", "LTV", "ARPU", "NPS", "ROI",
  "virality", "stickiness", "engagement", "funnel", "pipeline",
  "bottleneck", "optimization", "scale", "traction", "momentum",
  "disruption", "paradigm", "synergy", "leverage", "ecosystem",
  "moonshot", "10x", "rocketship", "hypergrowth", "blitzscaling",
  "product_market_fit", "go_to_market", "first_principles",
  "zero_to_one", "network_effects", "flywheel", "moat", "TAM",
  "SAM", "SOM", "KPI", "OKR", "A_B_test", "power_user"
];
...
class ASCIIEffect {
            ...
            if (Math.random() < 0.3) {
                ...
                for (let x = 0; x < this.width; x++) {
                    if (x >= offset && x < offset + word.length) {
                        ...
                        const char = Math.random() > 0.95 ? String.fromCharCode(33 + Math.floor(Math.random() * 94)) : ' ';
                        row.push(char);
                        originalRow.push(char);
                    }
                }
                ...
                for (let x = 0; x < this.width; x++) {
                    const char = Math.random() > 0.98 ? String.fromCharCode(33 + Math.floor(Math.random() * 94)) : ' ';
                    row.push(char);
                    originalRow.push(char);
                }
            }
    ...
    private updateGrid(): void {
        if (this.mousePos) {
            ...
            for (let y = Math.max(0, this.mousePos.y - radius); y < Math.min(this.height, this.mousePos.y + radius); y++) {
                for (let x = Math.max(0, this.mousePos.x - radius); x < Math.min(this.width, this.mousePos.x + radius); x++) {
                    const distance = Math.sqrt(Math.pow(x - this.mousePos.x, 2) + Math.pow(y - this.mousePos.y, 2));
                    ...
                    if (distance < radius) {
                        ...
                        if (Math.random() < changeProb) {
                            ...
                            this.grid[y][x] = randomWord[Math.floor(Math.random() * randomWord.length)];
                        }
                    } else {
    ...
    private render(): void {
        this.container.textContent = this.grid.map(row => row.join('')).join('\n');
    }
